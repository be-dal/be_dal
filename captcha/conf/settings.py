...
CAPTCHA_FONT_SIZE = getattr(settings,'CAPTCHA_FONT_SIZE', 50)
...
CAPTCHA_LENGTH = int(getattr(settings, 'CAPTCHA_LENGTH', 8)) # Chars
...
if CAPTCHA_IMAGE_BEFORE_FIELD:
    CAPTCHA_OUTPUT_FORMAT = getattr(settings,'CAPTCHA_OUTPUT_FORMAT', u'%(image)s %(hidden_field)s</br> %(text_field)s')
else:
    CAPTCHA_OUTPUT_FORMAT = getattr(settings,'CAPTCHA_OUTPUT_FORMAT', u'%(hidden_field)s %(text_field)s %(image)s')

######### EDITED #########
# change this if you want your captcha to be in capital letters or not
CAPTCHA_UPPER = True
